...
# for(i in 1:3){
  ...
  try({
    ...
    clim_mod_list <- clim_long$model_id %>% unique()
    ...
    # loop through clim mods ----
    for(clim_model_id in clim_mod_list){
      ...
      fc_climate_mods_efi <- fc_mods %>% 
        ...
        mutate(
          ...
          model_id = paste0("bet_",model_id, "_clim_ensemble"),
          parameter = paste0(clim_model_id, "_", parameter),
